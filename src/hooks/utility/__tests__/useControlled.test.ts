...
describe('useControlled tests', () => {
    it('should preset value when undefined', () => {
        expect(useControlled({ setState: () => null })).toMatchObject({ value: '' });
    });

    it('should use defaultValue when defined', () => {
        expect(useControlled({ setState: () => null, defaultValue: 'hello' })).toMatchObject({
            value: 'hello',
        });
    });

    it('should use value when defined', () => {
        expect(useControlled({ setState: () => null, value: 'hello' })).toMatchObject({
            value: 'hello',
        });
    });
